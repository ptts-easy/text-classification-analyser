...
import matchWord from 'word-regex';



// `createDoc` is used to count how often a specific word occurs.
export const createDoc = function () {
...
export const nbayes = function () {
	...
	return {
	    ...
	    for (let _class in config.wordsByClass) {
				wordsByClass[_class] = createDoc();
				for(let word in config.wordsByClass[_class]._words) {
					wordsByClass[_class].set(word, config.wordsByClass[_class]._words[word]);
				}
	    }
			docsByClass = config.docsByClass;
			words = createDoc();
			for(let word in config.words._words) {
				words.set(word, config.words._words[word]);
			}
			docs = config.docs;
		},
        ...
        wordsByClass: wordsByClass,
    	  words: words,
        docsByClass: docsByClass,
        docs: docs
      });
		},
